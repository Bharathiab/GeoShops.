...
export const createProperty = async (department, propertyData) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.details || 'Failed to create property');
    }
    return await response.json();
  } catch (error) {
...
export const updateHostProfile = async (id, data) => {
  try {
    const response = await fetch(`${API_URL}/hosts/${id}/profile`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    if (!response.ok) throw new Error('Failed to update host profile');
    return await response.json();
  } catch (error) {
...
export const createMembershipTemplate = async (data) => {
  try {
    ...
    console.error('Error creating membership template:', error);
    throw error;
  }
};
